...
describe("Folders — Happy", () => {
  ...
  it("FLD-HAPPY-003: GET /files/folders lists root", async () => {
    ...
    const res = await authed(app, session).get("/api/files/folders").expect(200);
  ...
  it("FLD-HAPPY-004: GET /files/folders/:id returns folder + breadcrumbs", async () => {
    ...
    const res = await authed(app, session).get(`/api/files/folders/${child}`).expect(200);
  ...
  it("FLD-HAPPY-007: DELETE folder moves to trash", async () => {
    ...
    const res = await authed(app, session).del(`/api/files/folders/${id}`).expect(200);
