...
describe("Files — Negative", () => {
  it("FILE-NEG-001: upload missing fields → 400", async () => {
    ...
    const res = await authed(app, session).post("/api/files/upload").send({}).expect(400);
  ...
  it("FILE-NEG-005: confirm when S3 HEAD missing → 400", async () => {
    ...
    const res = await authed(app, session)
      ...
      .send({ key: up.body.key, fileName: "z.txt", fileType: "text/plain", fileSize: 100 })
      .expect(400);
    expect(res.body.error).toBe("File not found in S3 — upload may have failed");
  });
  ...
  it("FILE-NEG-007: PATCH with no fields → 400", async () => {
    ...
    await authed(app, session).patch(`/api/files/files/${id}`).send({}).expect(400);
  });
  ...
  it("FILE-NEG-010: DELETE nonexistent → 404", async () => {
    const session = await loginUser(app, userA);
    await authed(app, session).del("/api/files/files/507f1f77bcf86cd799439011").expect(404);
  });

  it("FILE-NEG-011: favourite without body → 400", async () => {
    const session = await loginUser(app, userA);
    const id = await createFile(app, session);
    await authed(app, session).patch(`/api/files/files/${id}/favourite`).send({}).expect(400);
  });
