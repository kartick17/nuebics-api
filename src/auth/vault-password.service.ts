...
export class VaultPasswordService {
  ...
  async getVerifier(userId: string): Promise<string> {
    const user = await this.userModel
      .findById(userId)
      .select('vaultCredentialVerifier');
    if (!user) throw new NotFoundException('User not found.');
    if (!user.vaultCredentialVerifier)
      throw new NotFoundException('No vault password set.');
    const verifier = this.crypto.decryptToken(user.vaultCredentialVerifier);
    if (!verifier)
      throw new InternalServerErrorException('Vault password corrupted.');
    return verifier;
  }
  ...
  async setVerifier(userId: string, encryptedToken: string) {
    ...
    if (!user) throw new NotFoundException('User not found.');
    if (user.vaultCredentialVerifier) {
      const credentialChecker = this.crypto.decryptToken(
        user.vaultCredentialVerifier
      );
