...
export class FoldersService {
  ...
  async createFolder(userId: string, dto: CreateFolderInput) {
    ...
    if (parentId) {
      ...
      const parentFolder = await this.folderModel.findOne({ _id: parentId, userId });
      if (!parentFolder) {
    ...
    if (existing) {
      return { error: 'A folder with this name already exists here', status: 409 } as const;
    }
  ...
  async updateFolder(userId: string, id: string, dto: UpdateFolderInput) {
    ...
    if (name === undefined && parentId === undefined) {
      return { error: 'Nothing to update — provide name or parentId', status: 400 } as const;
    }
    ...
    if (parentId !== undefined) {
      ...
      if (newParentId === currentParent) {
        return { error: 'Folder is already in this location', status: 400 } as const;
      }
      ...
      if (newParentId !== null) {
        ...
        const isCircular = await this.helpers.isDescendantOf(newParentId, id, userId);
        if (isCircular) {
          return { error: 'Cannot move a folder into one of its subfolders', status: 400 } as const;
        }

        const targetParent = await this.folderModel.findOne({ _id: newParentId, userId });
        if (!targetParent) {
  ...
  async deleteFolder(userId: string, id: string) {
    ...
    const folder = await this.folderModel.findOne({ _id: id, userId, status: 'active' });
    if (!folder) {
      return { error: 'Folder not found', status: 404 } as const;
    }
    await this.helpers.trashFolderRecursive(id, userId);
    return { success: true, message: `${folder.name} moved to trash`, status: 200 } as const;
  }
